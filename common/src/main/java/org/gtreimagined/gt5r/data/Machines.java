...
import org.gtreimagined.gt5r.blockentity.multi.*;
import org.gtreimagined.gt5r.blockentity.single.*;
...
public class Machines {
     ...
     **/
    public static SteamMachine SOLID_FUEL_BOILER = new SteamMachine(GT5RRef.ID, "solid_fuel_boiler").setTiers(BRONZE, STEEL).setMap(RecipeMaps.SOLID_FUEL_BOILERS).addFlags(GUI, STEAM, ITEM, FLUID, CELL).baseTexture(Textures.BOILER_HANDLER).setTile(BlockEntityCoalBoiler::new).noCovers();
    ...
    public static BasicMachine SMALL_HEAT_EXCHANGER = new SecondaryOutputMachine(GT5RRef.ID, "small_heat_exchanger").setSecondaryOutputCover(GT5RCovers.COVER_OUTPUT_SECONDARY).removeFlags(EU).covers(ICover.emptyFactory, ICover.emptyFactory, ICover.emptyFactory, COVEROUTPUT, GT5RCovers.COVER_OUTPUT_SECONDARY, ICover.emptyFactory).setTiers(NONE).baseTexture(new Texture(GT5RRef.ID, "block/machine/base/small_heat_exchanger")).setMap(RecipeMaps.HEAT_EXCHANGER).addFlags(GUI, ITEM, FLUID).setTile(BlockEntitySmallHeatExchanger::new).frontCovers().allowFrontIO();
